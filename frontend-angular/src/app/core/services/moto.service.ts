// src/app/core/services/moto.service.ts
...
export class MotoService {
  ...
  /* ── Subjects (private write, public read) ── */
  ...
  get recentMotos(): Moto[] {
    return [...this._motos$.value]
      .sort((a, b) =>
        new Date(b.fechaRegistro ?? 0).getTime() - new Date(a.fechaRegistro ?? 0).getTime()
      )
      .slice(0, 5);
  }
  ...
  loadMotos(): void {
    ...
    this.http.get<Moto[]>(this.apiUrl).pipe(
      tap(data => {
        const motos = Array.isArray(data) ? data : [];
  ...
  createMoto(dto: MotoCreateDto): Observable<Moto> {
    return this.http.post<Moto>(this.apiUrl, dto).pipe(
      tap(() => this.loadMotos()),
